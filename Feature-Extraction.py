...
def combine_by_timerange_pandas(dname = 'r5.2'):
...
def getuserlist(dname = 'r5.2', psycho = True):
...
def get_mal_userdata(data = 'r5.2', usersdf = None):
...
def email_process(act, data = 'r5.2', separate_send_receive = True):
...
def http_process(act, data = 'r5.2'):
    # basic features:
...
def file_process(act, complete_ul = None, data = 'r5.2', filetype = 'act'):
    if filetype == 'act':
...
def process_week_num(week, users, userlist = 'all', data = 'r5.2'):
...
def proc_u_features(uf, ufdict, list_f = None, data = 'r5.2'): #to remove mode
    if type(list_f) != list:
        list_f=['project']
...
def f_calc(ud, mode = 'week', data = 'r5.2'):
    ...
    device_statf = ['usb_dur','file_tree_len']
...
if __name__ == "__main__":
    ...
    if dname not in ['r5.2']:
        raise Exception('Please put this script in and run it from a r5.2 CERT data folder')
